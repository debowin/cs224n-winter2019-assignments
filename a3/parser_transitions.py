...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise Exception("Invalid Transition: " + transition)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parse2index = {pp: i for i, pp in enumerate(partial_parses)}
    unfinished_parses = partial_parses[:]
    while unfinished_parses:
        minibatch_partial_parses = unfinished_parses[:batch_size]
        minibatch_transitions = model.predict(minibatch_partial_parses)
        for i, partial_parse in enumerate(minibatch_partial_parses):
            partial_parse.parse_step(minibatch_transitions[i])
            if not partial_parse.buffer and len(partial_parse.stack) == 1:
                dependencies[partial_parse2index[partial_parse]] = (partial_parse.dependencies)
